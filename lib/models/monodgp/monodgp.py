...
from utils.misc import NestedTensor, accuracy, get_world_size, inverse_sigmoid
...
class MonoDGP(nn.Module):
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        for lvl in range(hs.shape[0]):
            ...
            depth_ave = torch.cat(
                [
                    depth_geo.unsqueeze(-1) + depth_geo_err[:, :, 0:1],
                    depth_geo_err[:, :, 1:2],
                ],
                -1,
            )
...
class SetCriterion(nn.Module):
    ...
    def loss_region(self, outputs, targets, indices, num_boxes):
        ...
        for region_prob in region_probs:
            ...
            dice_coef = (2.0 * intersection + 1) / (total + 1)
            # Compute Dice Loss
    ...
    def loss_projection_alignment(self, outputs, targets, indices, num_boxes):
        ...
        bin_centers = (
            torch.arange(12, device=angle_cls.device, dtype=angle_cls.dtype)
            * angle_per_class
        )  # [12]
        ...
        x_corners = torch.stack(
            [ll / 2, ll / 2, -ll / 2, -ll / 2, ll / 2, ll / 2, -ll / 2, -ll / 2], dim=1
        )  # [M, 8]
        ...
        x_rot = (
            cos_ry.unsqueeze(1) * x_corners + sin_ry.unsqueeze(1) * z_corners
        )  # [M, 8]
        z_rot = (
            -sin_ry.unsqueeze(1) * x_corners + cos_ry.unsqueeze(1) * z_corners
        )  # [M, 8]
    ...
    def forward(self, outputs, targets, mask_dict=None):
        ...
        outputs_without_aux = {
            ...
            if k != "aux_outputs" and k != "inter_outputs"
        }
...
def build(cfg):
    ...
    for i in range(layers):
        inter_weight_dict.update(
            {k + f"_inter_{i}": v for k, v in weight_dict.items() if k in inter_keys}
        )
